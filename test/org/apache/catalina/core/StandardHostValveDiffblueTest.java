...
public class StandardHostValveDiffblueTest {
  /**
   * Test new {@link StandardHostValve} (default constructor).
   * <p>
   * Method under test: default or parameterless constructor of {@link StandardHostValve}
   */
  @Test
  public void testNewStandardHostValve() {
    // Arrange and Act
    StandardHostValve actualStandardHostValve = new StandardHostValve();

    // Assert
    assertEquals("Catalina", actualStandardHostValve.getDomain());
    assertEquals("NEW", actualStandardHostValve.getStateName());
    assertNull(actualStandardHostValve.getDomainInternal());
    assertNull(actualStandardHostValve.getObjectName());
    assertNull(actualStandardHostValve.getContainer());
    assertNull(actualStandardHostValve.getNext());
    assertEquals(0, actualStandardHostValve.findLifecycleListeners().length);
    assertEquals(LifecycleState.NEW, actualStandardHostValve.getState());
    assertTrue(actualStandardHostValve.getThrowOnFailure());
    assertTrue(actualStandardHostValve.isAsyncSupported());
  }

  /**
   * Test {@link StandardHostValve#invoke(Request, Response)}.
   * <ul>
   ...
   * </ul>
   * <p>
   * Method under test: {@link StandardHostValve#invoke(Request, Response)}
   */
  @Test
